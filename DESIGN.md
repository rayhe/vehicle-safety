...
- [x] Engineering vs Behavior Scatter Plot *(done: `scatter-plot.js` joins FARS_BY_MODEL with FARS_TOXICOLOGY to plot death rate (y) vs impairment % (x) for every vehicle with ≥100 tox drivers and ≥50K fleet. Dot size = total deaths, color = vehicle class. Quadrant shading reveals engineering failures (high rate, low impairment) vs behavior risk (high impairment, low rate). Median crosshairs, hover tooltips with full stats, auto-labels outliers. Dark mode support. Inserted above the By Model table.)*
